...
public:
    ...
    void drawCorners(fpx_t viewW, fpx_t viewH, fpx_t scale)
    {
        ...
        const auto  proj = Ui::Gl::Util::orthoProjection(cssW, cssH);
        ...
        for (id_t i = 0; i < 4; ++i) {
            ...
            Ui::Gl::Util::drawTriangles(verts, 6);
        }
    ...
    {
        cleanupCorners();
        Ui::Gl::Util::deleteProgram(m_cornerProgram);
        Ui::Gl::Util::deleteVertexArray(m_cornerVao);
...
private:
    void init()
    {
        ...
        const GLuint vert = Ui::Gl::Util::compileShader(GL_VERTEX_SHADER, vertSrc, "[Rounded] Vertex shader error: ");
        if (vert == 0) {
            return;
        }
        ...
        m_program = Ui::Gl::Util::linkProgram(vert, frag, "[Rounded] Link error: ");
        if (m_program == 0) {
            return;
        }
    ...
    void initCorner()
    {
        ...
        const GLuint vert = Ui::Gl::Util::compileShader(GL_VERTEX_SHADER, vertSrc, "[Rounded:Corner] Vert error: ");
        if (vert == 0) {
            return;
        }

        const GLuint frag = Ui::Gl::Util::compileShader(GL_FRAGMENT_SHADER, fragSrc, "[Rounded:Corner] Frag error: ");
        if (frag == 0) {
            glDeleteShader(vert);
            return;
        }

        m_cornerProgram = Ui::Gl::Util::linkProgram(vert, frag, "[Rounded:Corner] Link error: ");
        if (m_cornerProgram == 0) {
            return;
        }
        ...
        GLuint cornerVbo = 0;
        Ui::Gl::Util::createPosUvVao(m_cornerVao, cornerVbo);
        if (m_vbo == 0) {
            m_vbo = cornerVbo;
        } else {
            Ui::Gl::Util::deleteBuffer(cornerVbo);
        }
    }
