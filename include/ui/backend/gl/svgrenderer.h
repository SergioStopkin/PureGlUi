...
template <>
struct std::hash<Ui::Backend::Gl::cache_key_t> { // NOLINT(altera-struct-pack-align)
    std::size_t operator()(const Ui::Backend::Gl::cache_key_t & k) const
    {
...
private:
    ...
    void initGl()
    {
        ...
        m_program = Ui::Backend::Gl::Util::linkProgram(vert, frag, "[SVG] Shader link error: ");
        if (m_program == 0) {
            return;
        }
        ...
        Ui::Backend::Gl::Util::createPosUvVao(m_vao, m_vbo);
    }
    ...
    {
        Ui::Backend::Gl::Util::drawTriangles(Ui::Backend::Gl::Util::quadVertices(x, y, width, height), 6);
    }
                          ...
                          fpx_t                          shadowOpacity)
    {
        ...
        Ui::Backend::Gl::Util::forEachShadowLayer(
        shadowOffsetX,
        shadowOffsetY,
        blurRadius,
        shadowOpacity,
...
public:
    ...
    void begin(fpx_t viewWidth, fpx_t viewHeight, fpx_t scale)
    {
        ...
        const auto  projection = Ui::Backend::Gl::Util::orthoProjection(cssWidth, cssHeight);
    ...
    void clearCache()
    {
        for (auto & pair : textureCache_) {
            Ui::Backend::Gl::Util::deleteTexture(pair.second.textureId);
        }
        textureCache_.clear();
