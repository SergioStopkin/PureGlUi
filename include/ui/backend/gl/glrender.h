...
public:
    ...
    GlRender(std::function<void()> makeCurrent, const std::string & fontDir)
        : m_fontRenderer(std::make_unique<Ui::Backend::Gl::FontRenderer>(std::move(makeCurrent), fontDir))
    {
    }

    ~GlRender() override
    {
        Ui::Backend::Gl::Util::deleteProgram(m_flatProgram);
        Ui::Backend::Gl::Util::deleteBuffer(m_flatVbo);
        Ui::Backend::Gl::Util::deleteVertexArray(m_flatVao);
    }
    ...
    [[nodiscard]] Ui::Backend::Gl::FontRenderer * fontRenderer() { return m_fontRenderer.get(); }
                   ...
                   const Ui::Render::shadow_t & shadow) override
    {
        ...
        const std::string key = tinted ? Ui::Backend::Gl::SvgRenderer::loadFilledFromFile(path)
                                       : Ui::Backend::Gl::SvgRenderer::ensureLoaded(path);
        if (key.empty() || !Ui::Backend::Gl::SvgRenderer::isLoaded(key)) {
            return;
        }
                      ...
                      const Ui::Color & color) override
    {
        ...
        const auto projection = Ui::Backend::Gl::Util::orthoProjection(m_width / m_scale, m_height / m_scale);
        ...
        Ui::Backend::Gl::Util::drawTriangles(verts, 3);
...
private:
    ...
    void flushText()
    {
        ...
        Ui::Backend::Gl::FontRenderer::font_rec_t * fr       = nullptr;
        for (auto & item : m_textBatch) {
            if (item.font != prevFont) {
                ...
                const auto fontProj = Ui::Backend::Gl::Util::orthoProjection(m_width, m_height);
            ...
            const float        textW = Ui::Backend::Gl::FontRenderer::measureTextWidth(*fr, wtext);
            ...
            const Ui::fpx_t xCursor  = item.centered ? Ui::Backend::Gl::TextAlign::startXCenterClamped(item.pos,
                                                                                                      textWCss,
                                                                                                      item.minPadH,
                                                                                                      m_scale)
                                                     : Ui::Backend::Gl::TextAlign::startXLeft(item.pos, 0.0F, m_scale);

            m_vertexBuffer.clear();
            Ui::Backend::Gl::FontRenderer::appendTextVerts(m_vertexBuffer, *fr, wtext, xCursor, baseline);
            ...
            Ui::Backend::Gl::Util::drawTriangles(m_vertexBuffer);
        }
    ...
    void initFlatShader()
    {
        ...
        m_flatProgram = Ui::Backend::Gl::Util::linkProgram(vert, frag, "[Flat] Shader link error: ");
        if (m_flatProgram == 0) {
            return;
        }
    ...
    std::unique_ptr<Ui::Backend::Gl::FontRenderer> m_fontRenderer {};
