...
#include "ui/backend/window/windowbase.h"
#include "ui/config.h"
 ...
 *
 * Implements Ui::Backend::Window::IWindow for Linux using Wayland for window management and EGL for OpenGL context.
...
public:
    ...
    // -------- Ui::Backend::Window::IWindow implementation --------
    ...
    void resize(fpx_t width, fpx_t height) override
    {
        ...
        if (m_context) {
            auto * eglCtx = static_cast<Ui::Backend::Window::EglContext *>(m_context.get());
...
protected:
    ...
    std::unique_ptr<Ui::Backend::Window::IContext> m_context;
