...
- `.work/README.md` - index of the working notes (design targets, bug lists) with a STATUS per file. Read the index before any file in `.work/`: both design notes shipped most of their steps and deferred the last one, so "some of it is done" means read the rest
...
- `build.sh` - Build script: `./build.sh <dev|rel|test|ut|ct|cov>` (always use `rel` for builds; `test`/`ut`/`ct` enable `BUILD_TESTING` - `ut` and `ct` build a single suite each so a compile failure names it, `test` builds both)
- `run.sh` - Run script: `./run.sh <dev|rel|test|ut|ct> [args...]` (`ut`/`ct` run one suite; `test` runs both and sums the exit codes)
...
1. **common/ + ui/ vocabulary** (`include/common/`, `include/ui/`): `common/bit.h`, `common/json.h`, `common/sanitize.h`, `common/unicode.h`, `common/fs.h`, `common/system.h`, `common/backgroundworker.h`; `ui/type.h`, `ui/color.h`, `ui/config.h`, `ui/const.h`, `ui/convert.h`, `ui/codepoint.h`, `ui/registry.h`, `ui/index.h`, `ui/elementid.h`, `ui/tabbar.h`, `ui/action/registry.h`, `ui/action/actionmap.h`, `ui/intent.h`, `ui/intentkind.h`, `ui/result.h`, `ui/tab.h`, `ui/inithooks.h`, `ui/render/shadow.h`
2. **ui/ interfaces + render layer + GL backend** (`include/ui/`): `interface/irender.h`, `interface/ieventapp.h`, `interface/irenderer.h`, `interface/ichromecommands.h`, `render/uilayout.h`, `render/uirenderer.h`, `render/context.h`, `render/dockcolumn.h`, `render/uielement.h`, `render/uielementstate.h`, `render/elementevent.h`, `render/eventkind.h`, `render/binding.h`, `render/renderscope.h`, `render/elementstyle.h`, `render/axis.h`, `render/dragtrack.h`, `render/thumbmetrics.h`, `render/thumbtrack.h`, `render/scrollbar.h`, `render/slider.h`, `render/truncate.h`, `render/popup/popuprendererbase.h`, `render/popup/popuprenderer.h`, `render/popup/dialogrenderer.h`, `gl/localglew.h`, `gl/glutil.h`, `gl/glrender.h`, `gl/rounded.h`, `gl/svgrenderer.h`, `gl/fontrenderer.h`, `gl/fonttypes.h`, `gl/textalign.h`
...
3. **ui/res framework** (`include/ui/res/`): `resmanager.h`, `respath.h`, `util.h`, `localemanager.h`, `store/*.h` (the 7 sub-stores: icon/dialog/shortcut/layout/theme/menu/dock, plus the `menudisable.h` pass + `submenuentry.h` value), `type/*.h` (all UI value-type `*_t` structs + pure enums), `key/*.h` (res-key enums), `dock/*.h`
4. **ui/ runnable shell + actions + io** (`include/ui/`): `shell.h`, `action/exitapp.h`, `action/reload.h`, `action/switchtheme.h`, `action/switchthememode.h`, `action/openfile.h`, `io/filedialog.h`, `io/filefilter.h`; and the demo app `include/sig.h`, `src/main.cpp`
...
`Ui::Shell` (`ui/shell.h`) is the runnable framework core: it owns `Ui::Res::ResManager` + `Ui::Window::WindowManager` + `Ui::Render::Context` + `Ui::Action::Registry`, and provides the init spine, the run loop, the default shell actions, the event->intent dispatch, and all interactive-chrome orchestration (popup/menu/dialog lifecycle, temp status). It runs standalone: `Ui::Shell shell; shell.initialize(); shell.run();`. A host adds its domain steps as `Ui::init_hooks_t` (`ui/inithooks.h`) passed to `initialize()`; Shell keeps them and replays `loadDomainResources` + `gateFeatures` on every reload, so startup and reload chrome cannot diverge. The spine steps themselves are framework-internal - a host never drives them.

The **demo app** is a thin `src/main.cpp` (+ the header-only `include/sig.h`) that constructs a `Ui::Shell`, wires `sig` to `shell.requestStop()`, and runs. A real **host** would build on the framework the same way and add its domain on top via the public seams - it is not a separate include root here.
...
- Shell hooks: `setFrameTasks`/`setOnTick` (per-loop host work), `setOnTabActivated`/`setOnTabClosed`/`setOnKeyPress` (domain reactions), `setFileHandler(extension, fn)` (per-type file loaders for `OpenFile`), `setOnDockRowValue(fn(rowId, ratio))` (a dock slider moved - forwards to `WindowManager::setOnRowValue`; this is the seam a host calls).
...
Action subsystem (`ui/action/`, namespace `Ui::Action`): `Ui::Action::Registry` (`registry.h`) is the opaque `actionKey -> Ui::action_fn_t` dispatch table (`on`/`dispatch`/`has`). The built-in actions are one-per-header host-templated free functions (`exitapp.h`/`reload.h`/`switchtheme.h`/`switchthememode.h`/`openfile.h`); `actionmap.h::registerActions(shell)` binds them into the registry, called from the `Shell` CONSTRUCTOR so the registry is complete before a host registers anything (a host overrides a built-in key by calling `actions().on()` later - no ordering rule). A host registers its domain actions the same way via `shell.actions().on(key, fn)`. The `"action"` key comes from res JSON (menus/shortcuts); a menu item or toolbar button whose key has no registered handler is greyed out by the disable pass (`disableUnhandledItems`). There is no runtime name->function reflection in C++, so the key->function binding is an explicit list (see `.work/action-binding-reflection.md`).

The render-chrome layer lives in `ui/render/` (namespace `Ui::Render`): `UiLayout`/`UiRenderer`/`Context`/`DockColumn` + the value types (`UiElementType`/`UiElementState`/`element_event_t`), plus the popup-chrome renderers `ui/render/popup/` (`Ui::Render::Popup`: `PopupRendererBase`/`PopupRenderer`/`DialogRenderer`). The renderer/event CONTRACTS `Ui::IEventApp` + `Ui::IRenderer` live in `ui/interface/` (a host content renderer implements `Ui::IRenderer`). `UiRenderer` is window-free: it takes a `makeCurrent` callback + caches size.
...
- All structs use `_t` suffix naming: `bound_t`, `element_event_t`, `tab_t`
- All structs use `alignas()` specifier, e.g.: `struct alignas(128) row_t final`
...
- Never add new `#pragma clang diagnostic` (push/ignored/pop) blocks. If a warning fires, fix the root cause - pick a non-deprecated API, adjust the code, or suppress the warning project-wide in CMake if it's truly a systemic false positive. Silencing at the call site hides real signals. ONE existing exception is sanctioned (do NOT copy the pattern elsewhere): `-[NSOpenGLContext setView:]` in `ui/window/platform/macoswindow.h` (the entire NSOpenGL stack is deprecated in favour of Metal). A second exception for a `std::codecvt_utf8` wide-stream deprecation in `windowmanager.h` used to be listed here; that code is gone, and so is the pragma.
...
- Name an operation by what it does for the caller, never by how its body is written. The same verbs in both repos:
  - getter: bare noun (`layout()`, `themeName()`), or `isX()`/`hasX()` for a bool - never a `get` prefix
  - choose a value: method `setX(value)`, action `SetX` taking an argument
  - flip a bool: method `toggleX()`, action `ToggleX` with no argument, `X` being the bool's name without `is`
  - step through more than two states: `cycleX()` / `CycleX` (reserved until something needs it)
  - map one value to another, whatever the body is: `toX` names what it returns and `fromX` names what it takes, the other side implied (`toIdcCursor`, `toKeyModifier`); `xFromY` when neither side is implied; `xOf` for something derived from the argument; `xFor` for what applies to it
  - `switch` is not a naming verb: it used to mean both "choose" and "flip". Three names predate this rule and are due one rename together, because their action keys live in res menu JSON in both repos: `SwitchTheme` -> `SetTheme`, `SwitchTab` -> `SetActiveTab`, `SwitchThemeMode` -> `ToggleThemeMode`
...
- `registry.h` - `Registry<Key, T>`: generic ordered key-keyed container (unordered_map + order vector); one value per key.
- `index.h` - `Index<Key, T>`: group-by, many values per key, composed over `Registry`. For acceleration structures that would otherwise be a scan inside a loop (parent -> children over a tree).
- `elementid.h` - `Ui::ElementId` enum: element-id range starts (`MenuBase`/`ButtonBase`/`ItemBase`/`DockGripBase`/`DockScrollBase`/`DockRowBase`) the loader assigns from. `DockRowBase` is open-ended: the host supplies a row id and the framework only offsets it.
...
- `tab.h` - `tab_t`: tab view-model `{id,label,isActive,isLoading,progress}`.
...
- `interface/ieventapp.h` - `Ui::IEventApp`: pointer-event sink (onMouseMove/Press/Release/Leave/Scroll; press/release/scroll return `Ui::Render::element_event_t`, whose `changed` carries what a `bool` return used to mean). Press and release take a `Window::MouseButton` - every button is delivered, so an implementation must ignore what it does not handle rather than assume Left. Self-rooted. Base of `Ui::IRenderer` + `Ui::IWindow`/`Ui::Window::WindowManager`.
- `interface/ichromecommands.h` - `Ui::IChromeCommands`: the intents-in execution seam (emitAction/openPopup/closePopup/openDialog/switchTab/closeTab/copyText/activateRow/toggleRow/openRowMenu + isPopupOpen). `Ui::routeIntent(intent, chrome)` is the pure `IntentKind`->command switch; `Ui::Shell` privately implements the interface (forwarding to WindowManager + action registry + tab hooks) and `execute()` delegates to `routeIntent`. The counterpart to `Ui::Render::Context` (intents-out), and the seam headless UI tests record against.
...
- `render/uilayout.h` - `Ui::Render::UiLayout`: lays out the fixed UI regions into `UiElement`s, draws via `Ui::IRender`. `render/uirenderer.h` - `Ui::Render::UiRenderer`: walks the layout, emits IRender ops, owns the GL `GlRender`; window-free (makeCurrent callback + cached size). `render/context.h` - `Ui::Render::Context`: maps clicks/keys -> `Ui::intent_t`. `render/dockcolumn.h` - `Ui::Render::DockColumn`: one dock, its grip, its host-projected rows and its scrollbar. `render/uielement.h`/`uielementstate.h` - `Ui::Render` value types (`UiElementType`/`UiElementState`).
- The input model (`Ui::Render`): `render/eventkind.h` (`EventKind` - what can happen AND the per-element accept mask; `ANY_CLICK`/`ANY_DRAG`/`acceptsEvent`), `render/elementevent.h` (`element_event_t` - what happened to which element; replaced `click_result_t`), `render/binding.h` (`binding_t` + `defaultBinding` - the `(type, event) -> intent + scope` table; `MenuButton`/`MenuItem` are deliberately absent because their intent comes from res data), `render/renderscope.h` (`RenderScope` - None/Chrome/Layout/Content, the coarse repaint class; no per-dock value because a dock is not an OS surface), `render/elementstyle.h` (`element_style_t` - everything one element contributes to a frame, resolved in one pass).
- Thumb widgets (`Ui::Render`), shared by the dialog scrollbar, the dock scrollbar and the dock slider: `render/axis.h` (`Axis`), `render/dragtrack.h` (`drag_track_t` - every one-axis drag), `render/thumbmetrics.h` (`thumb_metrics_t` - track + thumb length + max, and the travel maths), `render/thumbtrack.h` (`thumb_track_t` - geometry + hover + drag in one place so drawing and pointer mapping cannot disagree), `render/scrollbar.h` (`ScrollBar`), `render/slider.h` (`sliderMetrics`). None of them own the VALUE or draw.
- `render/truncate.h` - `truncateText` / `truncateFileName`: fitting text to a box against a caller-supplied measuring sink. Stateless and layer-free; works on `wstring` internally so it slices by codepoint.
...
- `window/windowbase.h` - `WindowBase<Derived>` CRTP (owns the `Ui::IRenderer`, render-request wiring, geometry). `nativewindow.h`/`nativewindowhandle.h`/`nativedisplayhandle.h` - the `NativeWindow` alias + handle types (compile-time platform dispatch). `platform/{x11,wayland,macos,win32}window.h` - per-OS `IWindow` impls; `platform/*event.h` - per-OS `IEventOS` impls; `platform/x11include.h` - X11 header shim. `eglcontext.h` - `EglContext`: EGL context (X11/Wayland), the `IContext` impl. `event.h` (`Event`/`EventType`), `mousebutton.h` (`MouseButton`, X11 button numbering), `keymodifier.h` (`KeyModifier` + `hasModifier`), `pointershape.h` (`PointerShape` + `pointerShapeToThemeName`; set through `WindowManager::setCursor`), `eventfactory.h`, `clickcounter.h`.
- `window/windowmanager.h` - `Ui::Window::WindowManager`: the windowing coordinator (main window + popups/submenus/dialogs + event polling/dispatch + render queue + content-surface registry + Wayland compositing). Drives `[main window] + content surfaces` blind through `IWindow`/`IRenderer`/`IEventApp`; reads scale/compositing from `Ui::g_config`. App-facing seams are `std::function` setters (`setOnDialogClose`/`setOnElementHover`/`setDoubleClickConfig`/`setDialogContentResolver`/`setOnRowValue`) + accessors (`subscribe()`/`renderQueue()`/`mainWindow()`) + the popup/dialog/content-surface API + the dock-content API (`setDockRows(dockName, rows)`/`resetDockScroll`) and `setCursor(PointerShape)`. A dock slider's value comes back through `setOnRowValue` rather than as an intent: a drag is a continuous stream, not a discrete request.
...
- `store/iconstore.h` / `dialogstore.h` / `shortcutstore.h` / `layoutstore.h` / `themestore.h` / `menustore.h` / `dockstore.h` - `Ui::Res::Store::*`: the logical sub-loaders ResManager composes (icon defaults; dialog config; shortcuts; layout+popup; theme+name/mode+preview+scanThemeNames; menus/buttons/action-map/auto-submenu; session per-dock state). Loaders that diff return a `Changed` bit the facade ORs. Alongside them: `store/menudisable.h` (`disableUnhandled` for menus + buttons), `store/menuheight.h` (`menuHeightOf`, the one popup-height sum) and `store/submenuentry.h` (`submenu_entry_t`, the auto-submenu walker's intermediate).
- `type/*.h` - `Ui::Res::Type` UI value types: `bound`, `border`, `region`, `font`, `colorpair`, `changed`, `popup`, `theme`, `themepreview`, `layout`, `menu`, `button`, `dialog`, `icondefault`, `iconplace`, `alignh`/`alignv` (text placement, read from `text-align`/`vertical-align`; `AlignV::Center` is cap-height centred), `input` (UI scroll/key-anim half).
- `key/*.h` - `Ui::Res::Key`: one enum class per res-key domain, `<Domain>Key` in `<domain>.h` with a `<domain>KeyName()` mapper - the single source of every JSON key spelling the fw code references. `ElementKey`/`CssPropKey`/`LayoutKey`/`ThemeKey` each end in a `Count` sentinel (never a selector) so an exhaustive check counts against the enum, not a hand-kept number; each mapper carries a `case Count: break;` that keeps `-Wswitch` catching a real key with no spelling (`section` + `session` for session.json v2, `app`, `dock`, `icon` entry fields, `iconrole` for the fw-referenced icon roles). Keys only other res JSON references (menu icon roles, host-added keys) stay free-form strings - that set is open and data-driven. CSS-convention spellings stay kebab; the enums centralize, never re-spell.
- `dock/*.h` - `Ui::Res::Dock`: `anchor`, `config`, `layout`, `state`, `theme`, plus `row`/`rowkind` - `row_t` is one line of host-projected dock content (tree node, key/value property, group header, slider or menu choice in ONE type), with `id` opaque to the framework and `label`/`value` literal display text rather than locale keys. A `RowKind::Menu` row names a res menu key in `menu`; a click on its value opens that menu as a popup under the row, and the pick runs the menu item's own action.

### ui/ - runnable shell + actions + io (`Ui::`, `Ui::Action`, `Ui::Io`)

- `shell.h` - `Ui::Shell`: the runnable framework core. On macOS any TU including it must be built as Objective-C++ (it reaches Cocoa via `action/openfile.h` -> `io/filedialog.h`). Owns `ResManager` + `WindowManager` + `Context` + `Action::Registry`; provides `runApp(init_hooks_t)` (whole lifecycle) + `initialize(init_hooks_t)`/`run()`/`requestStop()`/`shutdown()`, the default-action binding (`Ui::Action::registerActions(*this)`, from the ctor), the event->intent dispatch, popup/menu/dialog lifecycle (`createMenuPopup`/`reloadChrome`/`reopenActivePopup`/`closePopupMenu`), the active-menu key save/restore across reload+move+resize, `disableUnhandledItems`, `resolveDialogPlaceholders` (%VERSION%/%CPU%/%GPU%/%GL%/%RAM%), and the host hooks (`setFrameTasks`/`setOnTick`/`setOnTabActivated`/`setOnTabClosed`/`setOnKeyPress`/`setFileHandler`). The load cycle (`loadAllResources` + `gateAndDisableUnhandled`) is private and shared by `initialize()` + `reloadChrome()`.
- `inithooks.h` - `Ui::init_hooks_t`: the host steps Shell weaves into the load cycle + init spine (`loadDomainResources`/`gateFeatures` - both replayed on every reload; the one-shot `afterWindowCreated` + `afterInit`; `afterReload`; `beforeExitSave`, which `runApp` calls between the loop's end and the exit session save). Named by purpose, not spine position.
...
- `include/sig.h` - `g_app_init(std::function<void()>)` + signal handling for graceful shutdown (stores a stop callback; not tied to any concrete app type). HEADER-ONLY: everything is `inline`, so there is no `sig.cpp` and `src/CMakeLists.txt` compiles only `main.cpp`.
- `src/main.cpp` - constructs a `Ui::Shell`, wires `sig` to `shell.requestStop()`, runs `initialize()` + `run()`. A real host adds content surfaces + domain actions + file handlers + domain hooks on top of the same `Ui::Shell`.
