...
1. **common/ + ui/ vocabulary** (`include/common/`, `include/ui/`): `common/bit.h`, `common/json.h`, `common/sanitize.h`, `common/unicode.h`, `common/fs.h`, `common/system.h`, `common/backgroundworker.h`; `ui/type.h`, `ui/color.h`, `ui/config.h`, `ui/const.h`, `ui/convert.h`, `ui/codepoint.h`, `ui/registry.h`, `ui/elementid.h`, `ui/tabbar.h`, `ui/action/registry.h`, `ui/action/actionmap.h`, `ui/intent.h`, `ui/intentkind.h`, `ui/result.h`, `ui/type/tab.h`, `ui/render/shadow.h`
2. **ui/ interfaces + render layer + GL backend** (`include/ui/`): `interface/irender.h`, `interface/ieventapp.h`, `interface/irenderer.h`, `interface/ipopuprenderer.h`, `render/uilayout.h`, `render/uirenderer.h`, `render/context.h`, `render/dockcolumn.h`, `render/uielement.h`, `render/uielementstate.h`, `render/clickresult.h`, `render/popup/popuprendererbase.h`, `render/popup/popupuirenderer.h`, `render/popup/dialogrenderer.h`, `backend/gl/localglew.h`, `backend/gl/glutil.h`, `backend/gl/glrender.h`, `backend/gl/rounded.h`, `backend/gl/svgrenderer.h`, `backend/gl/fontrenderer.h`, `backend/gl/fonttypes.h`, `backend/gl/textalign.h`
2b. **ui/ windowing (backend + coordinator) + pubsub** (`include/ui/`): `pubsub/subscribe.h`, `pubsub/subscribeid.h`, `backend/window/iwindow.h`, `backend/window/ieventos.h`, `backend/window/icontext.h`, `backend/window/windowbase.h`, `backend/window/nativewindow.h`, `backend/window/nativewindowhandle.h`, `backend/window/nativedisplayhandle.h`, `backend/window/event.h`, `backend/window/eventfactory.h`, `backend/window/clickcounter.h`, `backend/window/eglcontext.h`, `backend/window/platform/x11window.h`, `backend/window/platform/x11event.h`, `backend/window/platform/x11include.h` (+ wayland/macos/win32 window+event peers); the coordinator layer `window/windowmanager.h`, `window/renderqueue.h`, `window/compositetexture.h`, `window/contentsurface.h`, `window/contenthit.h`, `window/popup/popupwindow.h`, `window/popup/dialogwindow.h`
3. **ui/res framework** (`include/ui/res/`): `resmanager.h`, `respath.h`, `util.h`, `locale/localemanager.h`, `store/*.h` (the 7 sub-stores: icon/dialog/shortcut/layout/theme/menu/dock), `type/*.h` (all UI value-type `*_t` structs + pure enums), `dock/*.h`
...
- Never use partial namespace blocks; always use fully-qualified namespace paths (e.g. `Ui::Render::Popup::PopupUiRenderer`)
...
- `interface/ieventapp.h` - `Ui::IEventApp`: pointer-event sink (onMouseMove/Press/Release/Leave/Scroll, returns `Ui::Render::click_result_t`). Self-rooted. Base of `Ui::IRenderer` + `Ui::Backend::Window::IWindow`/`Ui::Window::WindowManager`.
...
- `interface/ipopuprenderer.h` - `Ui::IPopupRenderer : Ui::IRenderer`: adds corner-pixel upload (setCornerPixels/hasCornerPixels) + setAlpha for SDF software-rounded popup corners. Implemented by `PopupRendererBase`.
...
- `render/popup/*` - `Ui::Render::Popup`: `PopupRendererBase` (shared GL setup/text/SVG/corner mgmt + `IPopupRenderer` boilerplate, owns a `UiRenderer`), `PopupUiRenderer` (menu/submenu popups), `DialogRenderer` (modal dialogs: title/content/buttons/scroll/keyboard nav). Domain-blind chrome.
- `backend/gl/glrender.h` - `Ui::Backend::Gl::GlRender`: `IRender` impl over Rounded/Svg/Font + a flat shader; batches by mode, text font-sorted at endFrame.
- `backend/gl/rounded.h` - `Rounded`: SDF rounded-rect renderer + corner pixel capture (the ONLY path for rounded shapes).
- `backend/gl/svgrenderer.h` - `SvgRenderer`: SVG load/cache via librsvg/Cairo -> GL texture, premultiplied alpha.
- `backend/gl/fontrenderer.h` / `fonttypes.h` / `textalign.h` - FreeType atlas + text shader; `font_handle_t`/`font_metrics_t`; alignment enum.
- `backend/gl/glutil.h` - `Ui::Backend::Gl::Util`: shared GL helpers (shader compile/link, VAO, pos/uv layout).
- `backend/gl/localglew.h` - GL header shim (GLEW on Linux/Windows, `<OpenGL/gl3.h>` on macOS).

### ui/ - windowing (backend + coordinator) + pubsub (`Ui::Backend::Window`, `Ui::Window`, `Ui::PubSub`)

- `backend/window/iwindow.h` - `Ui::Backend::Window::IWindow`: the surface seam (native handles + create/resize/show/makeCurrent/swap + `render`/`refresh`/`requestRender`/`setRenderRequest`), a `Ui::IEventApp`. `ieventos.h` - `IEventOS`: OS event polling + clipboard (self-rooted). `icontext.h` - `IContext`: GL-context abstraction (self-rooted).
- `backend/window/windowbase.h` - `WindowBase<Derived>` CRTP (owns the `Ui::IRenderer`, render-request wiring, geometry). `nativewindow.h`/`nativewindowhandle.h`/`nativedisplayhandle.h` - the `NativeWindow` alias + handle types (compile-time platform dispatch). `platform/{x11,wayland,macos,win32}window.h` - per-OS `IWindow` impls; `platform/*event.h` - per-OS `IEventOS` impls; `platform/x11include.h` - X11 header shim. `eglcontext.h` - `EglContext`: EGL context (X11/Wayland), the `IContext` impl. `event.h` (`Event`/`EventType`/`MouseButton`/`KeyModifier`), `eventfactory.h`, `clickcounter.h`.
...
- `window/popup/popupwindow.h` - `Ui::Window::Popup::PopupWindow : Ui::Backend::Window::NativeWindow`: borderless always-on-top popup (X11 ARGB visual / Wayland xdg_popup / per-corner shape), owns a `Ui::Render::Popup` renderer. `followsParent()` reports when it is an X11 child of the main window (the server moves/re-anchors it, so the shell must not manually track move/resize). `window/popup/dialogwindow.h` - `DialogWindow : PopupWindow`: modal dialog (centering, close callback). Domain-blind.
...
- `locale/localemanager.h` - `Ui::Res::Locale::LocaleManager`: locale load + string lookup by JSON key.
