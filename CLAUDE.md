...
- `store/iconstore.h` / `dialogstore.h` / `shortcutstore.h` / `layoutstore.h` / `themestore.h` / `menustore.h` / `dockstore.h` - `Ui::Res::Store::*`: the logical sub-loaders ResManager composes (icon defaults; dialog config; shortcuts; layout+popup; theme+name/mode+preview+scanThemeNames; menus/buttons/action-map/auto-submenu; session per-dock state). Loaders that diff return a `Changed` bit the facade ORs. Alongside them: `store/menudisable.h` (`disableUnhandled` for menus + buttons) and `store/submenuentry.h` (`submenu_entry_t`, the auto-submenu walker's intermediate).
...
- `dock/*.h` - `Ui::Res::Dock`: `anchor`, `config`, `layout`, `state`, `theme`, plus `row`/`rowkind` - `row_t` is one line of host-projected dock content (tree node, key/value property, group header or slider in ONE type), with `id` opaque to the framework and `label`/`value` literal display text rather than locale keys.
...
- `inithooks.h` - `Ui::init_hooks_t`: the host steps Shell weaves into the load cycle + init spine (`loadDomainResources`/`gateFeatures` - both replayed on every reload; the one-shot `afterWindowCreated` + `afterInit`; `afterReload`). Named by purpose, not spine position.
