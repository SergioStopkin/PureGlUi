...
- `void appendElements(UiLayout & layout) const` / `void appendRowElements(UiLayout & layout) const` - contribute `DockGrip`, `DockRow`, `DockExpander`, `DockSlider`, `DockMenu` and `DockScrollbar` elements to the main-window layout, so dock content is hit-tested by the same pass as the chrome. `DockMenu` covers only a `Menu` row's value column and shares the row's element id, so a label click still activates the row.
- Menu rows: `std::string menuKeyOf(id_t rowId) const` - the res menu a `Menu` row opens, empty for any other row (and how `WindowManager::rowMenuKey(elementId)` finds the dock holding it).
...
- Narrow docks: nothing clips a dock's ops to its content, so what a row carries is drawn and offered for hit-testing only where it fits. A dock no wider than a row's padding either side shows no rows and no scrollbar; an expander or a menu chevron appears only inside its row, and a slider only when its track is at least one thumb long. Drawing and hit-testing ask the same question (`hasExpander` / `hasSlider`), so a narrow dock never draws a piece that does not answer.
